...
const HEATMAP_SCORE_COLOR: Record<number, string> = {
  5: "#c084fc", 4: "#a855f7", 3: "#7c3aed", 2: "#6d28d9", 1: "#5b21b6", 0: "#4c1d95",
};
...
function Donut({ slices, size = 140 }: { slices: { color: string; value: number }[]; size?: number }) {
...
function SparklineTooltip({ values, color, label, dateLabels, moodLabels }: {
  values: number[]; color: string; label: string;
  dateLabels?: string[]; moodLabels?: (string[] | null)[];
}) {
    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="relative">
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className="w-full overflow-visible"
          ...
          {ti !== null && <line x1={pts[ti][0]} y1={0} x2={pts[ti][0]} y2={H} stroke={color} strokeWidth="1" strokeDasharray="3 3" opacity="0.4" />}
          {pts.map(([x, y], i) => (
            <circle key={i} cx={x} cy={y} r={ti === i ? 6 : 3.5} fill={color} stroke="#1a1a1a" strokeWidth="1.5"
              style={{ cursor: "crosshair", transition: "r 0.1s" }}
              onMouseEnter={() => setTooltip({ idx: i })} />
          ))}
        </svg>
        {ti !== null && dateLabels && (
          <div className="absolute z-20 pointer-events-none px-2.5 py-1.5 rounded-xl text-xs shadow-2xl border border-gray-700 bg-[#111]"
            style={{
              left: `${(pts[ti][0] / W) * 100}%`,
            ...
            {moodLabels?.[ti]?.length
              ? <div className="flex gap-1.5 mt-0.5 flex-wrap">
                  {moodLabels[ti]!.map(m => { const meta = MOODS.find(x => x.label === m); return <span key={m} style={{ color: meta?.color ?? "#aaa" }} className="text-[11px]">{meta?.emoji} {m}</span>; })}
                </div>
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className="w-full">
        ...
        <path d={`${pathD} L ${pts[pts.length-1][0]} ${H} L ${pts[0][0]} ${H} Z`} fill={color} opacity="0.08" />
        <path d={pathD} fill="none" stroke={color} strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />
        {pts.map(([x, y], i) => <circle key={i} cx={x} cy={y} r="3" fill={color} stroke="#1a1a1a" strokeWidth="1.5" />)}
      </svg>
    </div>
  );
}
...
function Heatmap({ moodData }: { moodData: Record<string, any> }) {
  ...
  const cells = useMemo(() => Array.from({ length: days }, (_, i) => {
    ...
    return { key, color: score >= 0 ? (HEATMAP_SCORE_COLOR[score] ?? null) : null, label: meta ? `${meta.emoji} ${meta.label}` : "" };
  }), [moodData]);
  ...
  return (
    <div>
      ...
      <div className="flex gap-[2px] sm:gap-1 flex-wrap">
        {cells.map((c, i) => (
          <div key={i}
            className="w-3 h-3 sm:w-4 sm:h-4 rounded-sm transition-all duration-150 hover:scale-125 cursor-default"
      ...
      <div className="mt-2 flex items-center gap-1.5 sm:gap-2 flex-wrap">
        ...
        {[0,1,2,3,4,5].map(s => <div key={s} className="w-3 h-3 rounded-sm" style={{ background: HEATMAP_SCORE_COLOR[s] }} />)}
        <span className="text-[10px] text-gray-600 ml-0.5">More positive →</span>
      </div>
    </div>
  );
}
...
function getAvailableMonths(moodData: Record<string, any>) {
  ...
  Object.keys(moodData).forEach(date => { const [y, m] = date.split("-"); set.add(`${y}-${m.padStart(2,"0")}`); });
...
export default function MoodStats({ moodData }: { moodData: Record<string, any> }) {
  ...
  const filteredData = useMemo(() => {
    ...
    return Object.fromEntries(Object.entries(moodData).filter(([date]) => {
  ...
  const counts = useMemo(() => {
    ...
    entries.forEach(([, e]) => resolveMoods(e).forEach(label => { c[label] = (c[label] || 0) + 1; }));
    return c;
  }, [entries]);
  ...
  const tagCounts = useMemo(() => {
    ...
    return Object.entries(t).sort((a,b) => b[1]-a[1]).map(([tag, count]) => {
      const topMoodLabel = Object.entries(tagMoods[tag] ?? {}).sort((a,b) => b[1]-a[1])[0]?.[0] ?? null;
      return { tag, count, topMoodMeta: MOODS.find(m => m.label === topMoodLabel) };
    });
  }, [entries]);

  const { trend30, trend30Dates, trend30Moods } = useMemo(() => {
    const days = selectedMonth === "all" ? 30 : new Date(Number(selectedMonth.split("-")[0]), Number(selectedMonth.split("-")[1]), 0).getDate();
    const values: number[] = [], dates: string[] = [], moods: (string[] | null)[] = [];
  ...
  const moodTrends = useMemo(() => {
    const days = selectedMonth === "all" ? 14 : new Date(Number(selectedMonth.split("-")[0]), Number(selectedMonth.split("-")[1]), 0).getDate();
    return MOODS.map(m => ({
      ...m,
  ...
  const { avgScore, streak } = useMemo(() => {
    const scores = entries.map(([, e]) => { const moods = resolveMoods(e); return moods.length ? Math.max(...moods.map(m => SCORE[m] ?? 0)) : 0; });
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6 w-full">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <span className="text-xs text-gray-500 uppercase tracking-wider whitespace-nowrap">View:</span>
        <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <button onClick={() => setSelectedMonth("all")}
            className={`text-xs px-3 sm:px-4 py-1.5 rounded-full border whitespace-nowrap transition-all duration-200 flex-shrink-0
          ...
          {availableMonths.map(ym => (
            <button key={ym} onClick={() => setSelectedMonth(ym)}
              ...
              <span className="sm:hidden">{monthLabelShort(ym)}</span>
              <span className="hidden sm:inline">{monthLabel(ym)}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      {total === 0 ? (
        <div className="text-gray-500 text-sm text-center p-12 sm:p-16 bg-[#1a1a1a] rounded-2xl border border-dashed border-gray-700">
          {selectedMonth === "all" ? "No mood data yet. Start tracking!" : "No entries for this month."}
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            {[
              ...
              { num: avgScore, label: "Avg Score",  color: "text-cyan-400"  },
              { num: counts[topMood.label], label: `${topMood.emoji} ${topMood.label}`, color: "text-yellow-400" },
            ].map(s => (
              <div key={s.label} className="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-3 sm:p-4 text-center">
                <div className={`text-2xl sm:text-3xl font-bold ${s.color}`}>{s.num}</div>
                <div className="text-gray-500 text-[10px] sm:text-xs mt-1 uppercase tracking-wider leading-tight">{s.label}</div>
              </div>
            ))}
          </div>

          {/* ── Donut + Freq — stack on mobile ── */}
          <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-4 sm:p-5">
              <h3 className="text-white font-semibold text-xs sm:text-sm mb-3 sm:mb-4 uppercase tracking-wider">Mood Distribution</h3>
              <div className="flex items-center gap-3 sm:gap-5">
                <Donut slices={donutData} size={110} />
                <div className="flex flex-col gap-1.5 sm:gap-2 flex-1 min-w-0">
                  {MOODS.filter(m => counts[m.label] > 0).sort((a,b) => counts[b.label]-counts[a.label]).map(m => (
                    <div key={m.label} className="flex items-center gap-1.5 sm:gap-2 text-xs">
                      <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: m.color }} />
                      <span className="text-gray-300 flex-1 truncate">{m.emoji} {m.label}</span>
                      <span className="text-gray-500 font-mono text-[10px]">{counts[m.label]}x</span>
                      <span className="text-gray-600 text-[10px]">{Math.round((counts[m.label]/total)*100)}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-4 sm:p-5">
              <h3 className="text-white font-semibold text-xs sm:text-sm mb-3 sm:mb-4 uppercase tracking-wider">Frequency</h3>
          ...
          {/* ── Trend ── */}
          <div className="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-4 sm:p-5">
            <h3 className="text-white font-semibold text-xs sm:text-sm mb-1 uppercase tracking-wider">
              {selectedMonth === "all" ? "30-Day Trend" : `Trend — ${monthLabel(selectedMonth)}`}
            </h3>
            <p className="text-[10px] text-gray-600 mb-3">Hover points to see mood details</p>
            <SparklineTooltip values={trend30} color="#a78bfa" label="" dateLabels={trend30Dates} moodLabels={trend30Moods} />
            ...
            <div className="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-4 sm:p-5">
              <h3 className="text-white font-semibold text-xs sm:text-sm mb-3 sm:mb-4 uppercase tracking-wider">
                ...
                <span className="text-gray-600 normal-case font-normal text-xs ml-2">
                  {selectedMonth === "all" ? "(14 days)" : `(${monthLabel(selectedMonth)})`}
                </span>
              </h3>
              <div className="grid sm:grid-cols-2 gap-4 sm:gap-5">
                {moodTrends.map(m => <Sparkline key={m.label} values={m.values} color={m.color} label={`${m.emoji} ${m.label}`} />)}
              </div>
            </div>
          )}
          ...
          {selectedMonth === "all" && (
            <div className="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-4 sm:p-5">
              <h3 className="text-white font-semibold text-xs sm:text-sm mb-1 uppercase tracking-wider">12-Week Heatmap</h3>
              <p className="text-[10px] text-gray-500 mb-3 sm:mb-4">Each cell = 1 day</p>
          ...
          {tagCounts.length > 0 && (
            <div className="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-4 sm:p-5">
              <h3 className="text-white font-semibold text-xs sm:text-sm mb-3 sm:mb-4 uppercase tracking-wider">Top Tags</h3>
              <div className="flex flex-col gap-2 sm:gap-3">
                    ...
                    <div key={tag}>
                      <div className="flex justify-between items-center text-xs text-gray-400 mb-1">
                        <div className="flex items-center gap-1.5 sm:gap-2">
                          ...
                          {topMoodMeta && (
                            <span className="flex items-center gap-1 text-[10px] px-1.5 sm:px-2 py-0.5 rounded-full border"
