...
export default function Home() {
  ...
  useEffect(() => {
    setGrid(generateGrid(35));
  }, []);

  const handleCellClick = (idx: number) => {
    setGrid((prev) => {
      const next = [...prev];
      next[idx] = weightedRandom();
      return next;
    });
  };
  ...
  return (
    <div className="bg-[#0a0a0a] flex flex-col min-h-screen">
      ...
      <header className="bg-[#1a1a1a] border-b border-gray-800 shadow-lg sticky top-0 z-50">
        <nav className="container mx-auto px-6 py-3 flex items-center gap-4">
          <Image src="/logo.png" alt="Logo" width={50} height={50} className="filter brightness-110" />
          <div className="text-2xl font-bold text-purple-400">Mood Tracking Journal</div>
          <div className="ml-auto">
            <AuthDialog
              onLogin={setUser}
              customClass="bg-purple-500 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded-full transition duration-300"
      ...
      <main className="flex-grow">
        ...
        <section className="py-20 px-6">
          <div className="container mx-auto flex flex-col md:flex-row items-center gap-12 max-w-5xl">
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="inline-flex items-center gap-2 bg-purple-500/10 border border-purple-500/20 text-purple-300 text-xs font-semibold uppercase tracking-widest px-4 py-1.5 rounded-full mb-6">
              ...
              <div className="flex gap-3 justify-center md:justify-start flex-wrap">
                <button
                  className="bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 px-8 rounded-full transition duration-300 shadow-lg shadow-purple-500/30"
                ...
                <a
                  href="#features"
                  className="border border-gray-700 hover:border-gray-500 text-gray-400 hover:text-gray-200 font-medium py-3 px-8 rounded-full transition duration-300"
                >
                  How it works →
                </a>
              </div>
            ...
            <div className="flex-1 flex flex-col items-center gap-4">
              ...
              <div className="bg-[#1a1a1a] border border-gray-800 rounded-2xl p-5 shadow-xl isolate">
        ...
        {/* ── FEATURES ── */}
        <section id="features" className="py-16 px-6 border-t border-gray-800/60">
          <div className="container mx-auto max-w-5xl">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl font-bold text-white mb-3">Keep Track of Every Milestone</h2>
              <p className="text-gray-400 max-w-md mx-auto">Central intelligence for your daily goals — powered by smart statistics and beautiful visualizations.</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {FEATURES.map((f) => (
                <div
                  key={f.title}
                  className="bg-[#1a1a1a] border border-gray-800 hover:border-purple-500/40 rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1 group"
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-4 ${f.iconBg}`}>
                    {f.icon}
                  </div>
                  <h3 className="text-white font-bold text-lg mb-2 group-hover:text-purple-300 transition-colors duration-200">
                    {f.title}
                  </h3>
      ...
      {/* Footer */}
      <footer className="bg-[#1a1a1a] border-t border-gray-800 py-6 text-center">
        <div className="container mx-auto px-6">
          <p className="text-gray-500 text-sm">© 2026 Sirirat Puntamunee. All rights reserved.</p>
        </div>
      </footer>
